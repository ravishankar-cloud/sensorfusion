...
def show_pcl(pcl):
    ...
    visualising = o3d.visualization.VisualizerWithKeyCallback()
    visualising.create_window(window_name='Open3D', width=1200, height=1080, left=50, top=50, visible=True)
    
    global idx
    idx= True
    def right_click(visualising):
        global idx
        print('right key press')
        idx= False
        return
    visualising.register_key_callback(262,right_click)
    # step 2 : create instance of open3d point-cloud class
    pointcloud = o3d.geometry.PointCloud()
    # step 3 : set points in pointcloud instance by converting the point-cloud into 3d vectors (using open3d function Vector3dVector)
    pointcloud.points = o3d.utility.Vector3dVector(pcl[:,:3])
    # step 4 : for the first frame, add the pointcloud instance to visualization using add_geometry; for all other frames, use update_geometry instead
    visualising.add_geometry(pointcloud)
    # step 5 : visualize point cloud and keep window open until right-arrow is pressed (key-code 262)
    
    while idx:
        visualising.poll_events()
        visualising.update_renderer()
    #######
    ####### ID_S1_EX2 END #######     
